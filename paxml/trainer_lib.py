...
class _PjitPartitioner(Partitioner):
  ...
  def get_train_state_metadata(
    ...
    # partition spec, and reuse it for all subsequent partition() calls.
    if self._auto_sharding_result and partition_specs:
      # If partition_specs is provided, we want to make sure it's the same as
      # the one generated by auto-sharding.
      assert (
          self._auto_sharding_result.train_state_partition_spec
          == partition_specs
      )
      return train_state_metadata

    # In this case, we ignore the provided partition_specs.
    # TODO(laigd): get rid of the partition_specs arg. For auto-sharding, we
    # should use the cached spec, and for non-auto-sharding, we can
    # regenerate it every time (even though the result should be the same).
