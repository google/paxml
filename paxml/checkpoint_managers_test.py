...
def _create_reference_checkpoint_history(
    config_name, root_dir, checkpoint_type, steps, checkpoint_datetimes
):
...
class CheckpointManagerTest(parameterized.TestCase):
  ...
  def test_extract_latest_checkpoint_id(self):
    ...
    datetimes = [
        cdt,
        cdt + datetime.timedelta(hours=1),
  ...
  def test_save_no_max_to_keep(self, checkpoint_type):
    ...
    root_dir = os.path.join(
        FLAGS.test_tmpdir, 'test2', str(checkpoint_type), 'checkpoints'
    )
    tf.io.gfile.makedirs(root_dir)
  ...
  def test_save_max_to_keep(self, checkpoint_type):
    config_name = 'test.test_module.ConfigName'
    root_dir = os.path.join(
        FLAGS.test_tmpdir, 'test3', str(checkpoint_type), 'checkpoints'
    )
    tf.io.gfile.makedirs(root_dir)
    current_datetime = datetime.datetime.now(tz=datetime.timezone.utc)
    zero_datetime = datetime.datetime.fromtimestamp(0, tz=datetime.timezone.utc)
  ...
  def test_save_checkpoint_keep_interval_timedelta(self, checkpoint_type):
    config_name = 'test.test_module.ConfigName'
    root_dir = os.path.join(
        FLAGS.test_tmpdir, 'test4', str(checkpoint_type), 'checkpoints'
    )
    tf.io.gfile.makedirs(root_dir)
    current_datetime = datetime.datetime.now(tz=datetime.timezone.utc)
    zero_datetime = datetime.datetime.fromtimestamp(0, tz=datetime.timezone.utc)
  ...
  def test_save_restore_manager_case_1_default(self, checkpoint_type):
    config_name = 'test.test_module.ConfigName'
    root_dir = os.path.join(
        FLAGS.test_tmpdir, 'test5', str(checkpoint_type), 'checkpoints'
    )
    tf.io.gfile.makedirs(root_dir)
    current_datetime = datetime.datetime.now(tz=datetime.timezone.utc)
    zero_datetime = datetime.datetime.fromtimestamp(0, tz=datetime.timezone.utc)
  ...
  def test_save_restore_manager_case_2_mutant(self, checkpoint_type):
    config_name = 'test.test_module.ConfigName'
    root_dir = os.path.join(
        FLAGS.test_tmpdir, 'test6', str(checkpoint_type), 'checkpoints'
    )
    tf.io.gfile.makedirs(root_dir)
    current_datetime = datetime.datetime.now(tz=datetime.timezone.utc)
    zero_datetime = datetime.datetime.fromtimestamp(0, tz=datetime.timezone.utc)
  ...
  def test_save_on_preemption(self, checkpoint_type):
    config_name = 'test.test_module.ConfigName'
    root_dir = os.path.join(
        FLAGS.test_tmpdir, 'test7', str(checkpoint_type), 'checkpoints'
    )
    tf.io.gfile.makedirs(root_dir)
    current_datetime = datetime.datetime.now(tz=datetime.timezone.utc)
    zero_datetime = datetime.datetime.fromtimestamp(0, tz=datetime.timezone.utc)
    ...
    saved_checkpoint_datetimes = [
        checkpoint_datetimes[0],
        checkpoint_datetimes[save_step],
    ]
    filenames = [
        os.path.basename(v)
        for v in tf.io.gfile.glob(
