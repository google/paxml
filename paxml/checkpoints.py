...
def _is_tmp_checkpoint(directory: epath.Path, file: epath.Path) -> bool:
  ...
  return _is_tmp_checkpoint_asset(file) or (
      ...
      and not orbax.checkpoint.utils.is_checkpoint_finalized(directory / file)
  )


def _to_timestamp(datetime_instance: datetime.datetime) -> int:
  """Converts a datetime instance into an int timestamp."""
  timedelta = datetime_instance - datetime.datetime.fromtimestamp(
...
def _make_tmp_checkpoint_dir(
    checkpoint_dir: epath.Path, step: int, sync_timestamp: bool = False
) -> epath.Path:
  timestamp = _to_timestamp(datetime.datetime.now(tz=datetime.timezone.utc))
  if sync_timestamp:
    timestamp = multihost_utils.broadcast_one_to_all(np.array(timestamp))
    multihost_utils.assert_equal(
        timestamp, "Timestamps across hosts don't match."
    )
...
def save_checkpoint(
    ...
    async_ckpt_manager: Optional[
        gda_serialization.GlobalAsyncCheckpointManagerBase
    ] = None,
  ...
  if checkpoint_type == CheckpointType.CHECKPOINT_GDA:
    if use_orbax:
      ...
      _save_checkpoint_gda(
          train_state, checkpoint_dir, overwrite, step, async_ckpt_manager
      )
  ...
  if checkpoint_type == CheckpointType.CHECKPOINT_GDA:
    if use_orbax:
      ...
      return _restore_checkpoint_gda(
          state_global_shapes, checkpoint_dir, global_mesh, state_specs, step
      )
  elif checkpoint_type == CheckpointType.CHECKPOINT_FLAX:
    if use_orbax:
      checkpointer = FlaxCheckpointer()
...
def delete_temp_directories(
    checkpoint_dir: epath.Path, overwrite: bool = False
):
  ...
  if not overwrite and checkpoint_dir.exists():
    ...
    if jax.process_index() == 0:
      ...
      if tmp_checkpoint_dirnames:
        logging.warn(
            'Found incompletely saved checkpoints %s; deleting them',
            tmp_checkpoint_dirnames,
        )
    ...
    py_utils.sync_global_devices(
        'Wait for checkpoint tmp dir deletions to finish.'
    )
  ...
  if async_ckpt_manager is not None:
    async_ckpt_manager.serialize(
        ...
        on_commit_callback=functools.partial(
            on_commit_callback, checkpoint_step_tmp_dir, checkpoint_step_dir
        ),
    )
    ...
    py_utils.sync_global_devices(
        f'Finished saving GDA checkpoint for step {step} to'
...
def _restore_checkpoint_gda(
  ...
  if step is None:
    ...
    if tmp_checkpoint_dirnames:
      logging.warn(
          'Found incompletely saved checkpoints %s; skipping them',
          tmp_checkpoint_dirnames,
      )
    sorted_dirnames = sorted(
  ...
  if (
      checkpoint_step_dir.as_uri().startswith('gs://')
      and not (checkpoint_step_dir / COMMIT_SUCCESS_FILE).exists()
  ):
    raise ValueError(
        'The checkpoint save failed since the commit_success.txt '
  ...
  flattened_global_shapes = jax.tree_map(
      lambda x: x.shape, flattened_train_state
  )
  flattened_global_dtypes = jax.tree_map(
      lambda x: x.dtype, flattened_train_state
  )
  ...
  logging.info(
      'GDA checkpoint restore partition_spec_leaves: %s', flattened_state_specs
  )

  shardings = [
      jax.sharding.NamedSharding(global_mesh, s) for s in flattened_state_specs
  ]
  ...
  restored_train_state = _tensorstore_reconstruct(
      state_global_shapes, train_state_gda
  )
