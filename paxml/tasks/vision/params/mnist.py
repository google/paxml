...
def _get_dataset(batch_size, is_training=True):
  """Gets MNIST dataset."""
...
class MnistDatasetInput(base_input.BaseInput):
  """Pax MNIST data input."""

  def __post_init__(self):
    super().__post_init__()

    self.dataset: tf.data.Dataset = _get_dataset(
        self.batch_size, is_training=self.is_training
    ).map(lambda inputs, labels: NestedMap(inputs=inputs, labels=labels))
    self._iterator = self.dataset.as_numpy_iterator()
