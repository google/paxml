...
class CheckpointManager:
  ...
  def __init__(
    ...
    self._keep_interval_timedelta: Optional[
        datetime.timedelta
    ] = keep_interval_timedelta
  ...
  def should_save(self, global_step_id: int) -> bool:
    ...
    if (
        ...
        and multihost_utils.reached_preemption_sync_point(global_step_id)
    ):
      return True
    ...
    should_save_ckpt = (self._last_saved_checkpoint_step is None) or (
        ...
        and global_step_id % save_interval_steps == 0
    )
    # Whether to save a periodic checkpoint
    return should_save_ckpt

  def save_metadata(self, global_step_id: int, force: bool = False) -> None:
    ...
    Args:
      ...
      force: When set, do not call should_save() to determine whether this is a
        valid request to save a checkpoint or not.
    ...
    if self._keep_interval_timedelta is None:
      ...
      for checkpoint in self._checkpoint_history.checkpoints:
        if oldest_kept_timestamp is None or (
            (
                from_timestamp(oldest_kept_timestamp)
  ...
  def _delete_pattern_if_exists(
      self, root_dir: epath.Path, filepath: str
  ) -> None:
  ...
  def _delete_checkpoint(
      self, checkpoint: checkpoint_pb2.CheckpointMetadata
  ) -> None:
    """Deletes the checkpoint files for a given checkpoint."""
    logging.info(
        'Deleting checkpoint: %s %s', checkpoint.timestamp_sec, self._root_dir
    )
    if (
        self._checkpoint_history.checkpoint_type
  ...
  def _sweep(self, global_step_id: int) -> None:
    ...
    maybe_delete_checkpoints = []
    for checkpoint in self._checkpoint_history.checkpoints:
      if self._last_kept_checkpoint_datetime is not None and (
          from_timestamp(checkpoint.timestamp_sec)
    ...
    py_utils.sync_global_devices(
        f'checkpoint_manager:begin_delete_checkpoints:step_{global_step_id}'
    )
    while len(maybe_delete_checkpoints) > self._max_to_keep:
      ...
      if self._keep_interval_timedelta and (
          not kept_checkpoints
          or self._last_kept_checkpoint_datetime is None
          or (
              (
                  from_timestamp(checkpoint.timestamp_sec)
                  - self._last_kept_checkpoint_datetime
      ...
      self._delete_checkpoint(checkpoint)
    py_utils.sync_global_devices(
        f'checkpoint_manager:end_delete_checkpoints:step_{global_step_id}'
    )

    self._checkpoint_history = self._create_checkpoint_history()
    for c in itertools.chain(kept_checkpoints, maybe_delete_checkpoints):
      logging.info(
          'Keeping checkpoint: (step: %d, timestamp: %s)',
          c.global_step_id,
          c.timestamp_sec,
      )
      self._checkpoint_history.checkpoints.add().CopyFrom(c)
